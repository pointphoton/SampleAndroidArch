...
public abstract class NetworkBoundResource<ResultType, RequestType> {
   // protected Class<ResultType> clazz;
    ...
    NetworkBoundResource(AppExecutors appExecutors) {
        ...
        result.addSource(dbSource, data -> {
            ...
            if (shouldFetch(data)) {
                DebugLog.write(MessageFormat.format("data -> {0}",data==null?"null":data.toString()));
                fetchFromNetwork(dbSource);
            } else {
                result.addSource(dbSource, newData -> {
                    DebugLog.write(MessageFormat.format("newData -> {0}",newData==null?"null":newData.toString()));
    ...
    private void setValue(Resource<ResultType> newValue) {
        DebugLog.write(MessageFormat.format("MLiveData result.getValue() : {0} newValue : {1}",result.getValue(),newValue==null?"null":newValue.toString()));
